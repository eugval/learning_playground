...
if __name__ == '__main__':
    ...
    wait_for = None ## None  # None  # Add process ID to wait for it to finish before launching  23389
    ...
    current_at_bottleneckrgb_path = os.path.join(dataset_path, 'current_at_bottleneckrgb_data_paths.pckl' )
    current_at_bottleneckrgb_with_background_path = os.path.join(dataset_path, 'current_at_bottleneckrgb_with_background_data_paths.pckl' )
    current_at_bottleneckseg_from_network_path = os.path.join(dataset_path, 'current_at_bottleneckseg_from_network_data_paths.pckl' )
    current_at_bottleneckobject_seg_path = os.path.join(dataset_path, 'current_at_bottleneckobject_seg_data_paths.pckl' )
    current_at_bottleneckdistractor_seg_path = os.path.join(dataset_path, 'current_at_bottleneckdistractor_seg_data_paths.pckl' )
    current_at_bottleneckfull_seg_path = os.path.join(dataset_path, 'current_at_bottleneckfull_seg_data_paths.pckl' )
    current_at_bottlenecknorm_path = os.path.join(dataset_path, 'current_at_bottlenecknorm_data_paths.pckl' )
    current_at_bottleneckdepth_path = os.path.join(dataset_path, 'current_at_bottleneckdepth_data_paths.pckl' )
    ...
    cnn_params  = {'input_dimensions': [128,128],
                   'encoder_channels':  [3, 64, 64, 64, 64, 64, 64],   # [3, 64, 64, 64, 64, 64, 64], # [3, 32, 64, 128, 128, 256],
                   'regressor_layers': [128, 128, 128, 3],    # [128, 128, 128, 4], #[512, 128, 128, 128], [16,16,16]
                    'dropout' : 0.2,
                   'double_conv': True,
    ...
    # checkpoint_path = os.path.join(MODELS_PATH, 'direct_regression/direct_regression' )
    ...
    network_propagator = DirectRegPropagator(loss_func=loss,
                                       ...
                                       networks={'model': model},
                                        segmentation_use='segment', # 'segment', 'none' , 'concat'
                                            ...
                                            output = [ 'scales', 'pixel_deltas'],  # ['z_angles'],   # ['z_angles', 'scales', 'pixel_deltas'], ['scales', 'pixel_deltas']
                                        )
