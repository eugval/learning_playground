...
from dense_correspondence_control.learning.networks.Unet import UnetWithAuxHead
from dense_correspondence_control.learning.training.losses import ContrastiveDonLossWithSeg, calculate_focal_loss_alpha
from dense_correspondence_control.learning.training.propagators.don_propagator_with_seg import DonPropagatorWithSeg
from dense_correspondence_control.learning.training.trainer import Trainer
from dense_correspondence_control.learning.training.evaluation_functions.don_evaluation_function_with_seg import test_don, don_in_training_evaluation

SEED = 0



#MAsk rcnn explanaation https://fractaldle.medium.com/mask-r-cnn-unmasked-c029aa2f1296
...
if __name__ == '__main__':
    ...
    ## Training parameters
    dataset_name = "cup_randomised"# "cup_1k_tiny_transl  # "cup_4d_1k # cup_1k_only_rot  cup_1k_only_rot
    ...
    save_name = 'DON_with_seg_randomised'
    #Note: no norm here
    ...
    image_data_folders = [image_path, image_path,seg_path,seg_path]
    image_data_names = ["rgb_from", "rgb_to", "seg_from", "seg_to"]
    image_filename_paths = [from_image_names, to_image_names, from_image_names, to_image_names]
    image_data_types = ['.png', '.png', '.png', '.png']
    ...
    Unet_params  = {'encoder_channels' :  [3, 32, 64, 128, 256, 256, 512],
                    ...
                    'aux_channels': [3, 1], 'aux_kernels': [1], 'aux_strides': [1],
                    'aux_double_convs': False, 'aux_final_activation' : 'sigmoid', 'aux_bias': False,
    ...
    # Define Networks
    model = UnetWithAuxHead(**copy.deepcopy(Unet_params))
    model.to(device)
    ...
    # Define Loss
    alpha = calculate_focal_loss_alpha(seg_path)
    loss = ContrastiveDonLossWithSeg(distance_metric='double',
                                     margin_bg= 15.,
                                     margin_obj= 15.,
    ...
    testing_hyperparameters['model']['class'] = UnetWithAuxHead
    ...
    # Launch Training
    network_propagator = DonPropagatorWithSeg(loss_func=loss,
                                       optimiser=optimiser,
                                       networks={'model': model},
                                        object_loss_only=False)
    ...
    trainer = Trainer(network_propagator=network_propagator,
                      ...
                      extra_evaluation_params={'image_save_path': os.path.join(MODELS_PATH,save_name, 'image_examples'),
                                               'seg_save_path': os.path.join(MODELS_PATH,save_name, 'seg_examples')},
                      save_name=save_name,
